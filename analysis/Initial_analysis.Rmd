...
### measurement invariance in with loglik'
mag niet want allemaal dependetn 

```{r}

full_model <- sirt::lsem.estimate(
  data_base,
  moderator = "inclusiejaar",
  moderator.grid = moderator_grid,
  lavmodel = lavmodel_int,
  h = 2 ,
  fit_measures = c("aic", "df", "chisq", "logl"),
  est_joint  = TRUE,
  par_invariant = c("F=~ESR_conv")
)

summary(full_model)

metricinvariant_model <- sirt::lsem.estimate(
  data_base,
  moderator = "inclusiejaar",
  moderator.grid = moderator_grid,
  lavmodel = lavmodel_int,
  ...
  fit_measures = c("aic", "df", "chisq", "logl"),
  est_joint  = TRUE,
  par_invariant = c("F=~GH_conv", "F=~TJC_conv","F=~SJC_conv")
) 

summary(metricinvariant_model)


scalarinvariant_model <- sirt::lsem.estimate(
  data_base,
  moderator = "inclusiejaar",
  moderator.grid = moderator_grid,
  lavmodel = lavmodel_int,
  ...
  est_joint  = TRUE,
  par_invariant = c("F=~GH_conv", "F=~TJC_conv","F=~SJC_conv",
                    "ESR_conv~1", "SJC_conv~1", "TJC_conv~1", "GH_conv~1")
) 

df_metric_invar <- metricinvariant_model$fitstats_joint["df","value"] - 
  full_model$fitstats_joint["df","value"]

teststat_metric_invar <-
  -2*(metricinvariant_model$fitstats_joint["logl","value"] - 
        full_model$fitstats_joint["logl","value"] 
) 

df_scalar_invar <- scalarinvariant_model$fitstats_joint["df","value"] - 
  metricinvariant_model$fitstats_joint["df","value"]

teststat_scalar_invar <-
  -2*(scalarinvariant_model$fitstats_joint["logl","value"] - 
        metricinvariant_model$fitstats_joint["logl","value"] 
) 

pchisq(teststat_metric_invar,df=df_metric_invar,lower.tail=FALSE)
pchisq(teststat_scalar_invar,df=df_scalar_invar,lower.tail=FALSE)
